...
3. **本以外の情報源は `media/` に置く**。放送・番組など終点のない継続的なソースは、書籍と同じ構造では扱えない。
4. **本を横断する概念は `topics/` に切り出す**。ここが蓄積の本体。読んだ冊数が増えるほど、「本ごとのメモ」より「概念ごとのノート」に価値が集まる。
5. **プレーンなMarkdown + Git**。ツールに依存せず、差分が読め、grepできる。
6. **索引は自動生成**（`scripts/build-index.py`）。手でメンテすると必ず腐る。
...
media/                本以外の情報源（放送・番組など）
  README.md           メディア一覧（手動）
  <メディア名>/
    index.md          そのメディアが何か
    <特集名>.md        シリーズ・特集ごとの回別ノート
topics/               本を横断する概念ノート
